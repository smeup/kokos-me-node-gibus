...
(async () => {
  ...
  if (
    ...
    !options.includes("assiemi") &&
    !options.includes("rules") &&
    !options.includes("rulesPath")
  ) {
    console.log(
  ...
  if (options.includes("assiemi")) {
    ...
    const rulesPath: string =
      process.argv[process.argv.indexOf("--rulesPath") + 1];
    if (!rulesPath) {
      console.error(
        "The --rulesPath option requires a file path containing the rules to convert, one rule per line"
      );
      process.exit(1);
    } else {
      const rulesPath: string =
  ...
  const ruleDao = new RuleDaoProduction(
    consts.host,
    consts.user,
    consts.password,
    filter,
  ...
  const app = new RuleConverterApp(
    ruleDao,
    ruleConverterService,
    conversionResultValidator,
    conversionResultDao
  );
