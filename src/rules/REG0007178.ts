...
export const REG0007178: Rule = (iv) => {
    ...
    // #6
    // COND:
    // (( §FS_FAM2_ISOLA=§SI ) OR ( §FS_FAM2_ADDF=§SI )) AND ( §FS_ACC_CEN=§SI ) AND ( §FS_ACC_FRO=§SI )                              AND (( §FS_FAM2_TWIST=§SI ) OR ( §FS_FAM2_ZENIT=§SI ))
    // THEN:
    // *SET *CF=#1
    // #7
    // COND:
    ...
    // #8
    // COND:
    // ( §FS_FAM2_ADDL=§SI ) AND ( §FS_ACC_INI=§SI ) AND ( §FS_ACC_PRI_02=§SI ) AND ( ( §FS_FAM2_TWIST=§SI ) OR                       ( §FS_FAM2_ZENIT =§SI ) )
    // THEN:
    // *SET *CF=#1
    // #9
    // COND:
    // ( §FS_FAM2_ADDL=§SI ) AND ( §FS_ACC_CEN=§SI ) AND ( §FS_ACC_FRO=§SI ) AND (( §FS_FAM2_TWIST=§SI ) OR ( §FS_FAM2_ZENIT=§SI ))
    // THEN:
    // *SET *CF=#1
    // #10
    // COND:
    ...
    //#6
    if ((vars.get('§FS_FAM2_ISOLA') == vars.get('§SI') || vars.get('§FS_FAM2_ADDF') == vars.get('§SI')) && 
        vars.get('§FS_ACC_CEN') == vars.get('§SI') && vars.get('§FS_ACC_FRO') == vars.get('§SI') &&
        (vars.get('§FS_FAM2_TWIST') == vars.get('§SI') || vars.get('§FS_FAM2_ZENIT') == vars.get('§SI'))) {
        vars.setCF(1);
    }
