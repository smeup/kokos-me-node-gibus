...
export const REG0004707: Rule = (iv) => {
    ...
    // #1
    // COND:
    // ( §FS_FAM2_ISOLA=§SI ) AND (§FS_ACC_PRI<>§SI ) AND (§FS_ACC_SX_DX<>§SI) AND (§FS_ACC_CEN<>§SI )
    // THEN:
    // *SET *CON-A=*COL
    // *SET *CON-B=*LUNG
    // *SET *S2=*LUNG
    ...
    // RESPONSE:
    //#1
    if (vars.get('§FS_FAM2_ISOLA') == vars.get('§SI') && vars.get('§FS_ACC_PRI') != vars.get('§SI') && vars.get('§FS_ACC_SX_DX') != vars.get('§SI') && vars.get('§FS_ACC_CEN') != vars.get('§SI')) {
