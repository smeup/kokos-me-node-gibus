...
export const REG0004006: Rule = (iv) => {
    ...
    // #1
    // COND:
    // ( §H_POS_M1=#0 ) AND ( §FS_FAM2_ADDL= §SI ) AND (§FS_ACC_CEN<>§SI)
    // THEN:
    // *SET *CF=#0
    // #2
    // COND:
    ...
    //#1
    if ((vars.get('§H_POS_M1') == 0) && (vars.get('§FS_FAM2_ADDL') == vars.get('§SI')) && (vars.get('§FS_ACC_CEN') != vars.get('§SI'))) {
        vars.setCF(0);
    }
    //#2
