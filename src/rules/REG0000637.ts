...
export const REG0000637: Rule = (iv) => {
    ...
    // #1
    // COND:
    // (§FS_TOLO_GC = §SI)  OR (§FS_SP_0_GCT = §SI) OR (§FS_13_QU_GC = §SI)
    // THEN:
    ...
    // RESPONSE:
    //#1
    if (vars.get('§FS_TOLO_GC') == vars.get('§SI') || vars.get('§FS_SP_0_GCT') == vars.get('§SI') || vars.get('§FS_13_QU_GC') == vars.get('§SI')) {
