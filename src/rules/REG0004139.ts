...
export const REG0004139: Rule = (iv) => {
    ...
    // #2
    // COND:
    // ( §FS_ACC_ANT_DX=§SI ) AND ( §FS_ACC_INI = §SI )
    // THEN:
    // *SET *CF=#1
    // #3
    // COND:
    ...
    // #5
    // COND:
    // ( §FS_ACC_ANT_SX=§SI ) AND ( §FS_ACC_FIN = §SI )
    // """
    // RESPONSE:
    //#1
    ...
    //#2
    if ((vars.get('§FS_ACC_ANT_DX') == vars.get('§SI')) && (vars.get('§FS_ACC_INI') == vars.get('§SI'))) {
        vars.setCF(1);
    }
    ...
    //#5
    if ((vars.get('§FS_ACC_ANT_SX') == vars.get('§SI')) && (vars.get('§FS_ACC_FIN') == vars.get('§SI'))) {
    }
    // GENERATED
