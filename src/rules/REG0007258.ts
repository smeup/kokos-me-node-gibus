...
export const REG0007258: Rule = (iv) => {
    ...
    // #2
    // COND:
    // ( §FS_ACC_PRI_02=§SI ) AND ( §FS_MON_ANT1=§NO ) AND ( §TI_ANG_ANT1='01' )
    // THEN:
    // *SET *CF=#1
    // #3
    // COND:
    ...
    vars.setCF(0);
    //#2
    if ((vars.get('§FS_ACC_PRI_02') == vars.get('§SI')) && (vars.get('§FS_MON_ANT1') == vars.get('§NO')) && (vars.get('§TI_ANG_ANT1') == '01')) {
        vars.setCF(1);
    }
    //#3
