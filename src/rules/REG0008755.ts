...
export const REG0008755: Rule = (iv) => {
    ...
    // #5
    // COND:
    // §FS_FAM2_TWIST = §SI AND §FS_STA_MUR_ANT1 = §SI AND §VER_STA_T01 <> '2021'
    // THEN:
    // *SET *CF = #0
    // #6
    // COND:
    ...
    //#5
    if (vars.get('§FS_FAM2_TWIST') == vars.get('§SI') && vars.get('§FS_STA_MUR_ANT1') == vars.get('§SI') && vars.get('§VER_STA_T01') != '2021') {
        vars.setCF(0);
    }
    //#6
