...
export const REG0004804: Rule = (iv) => {
    ...
    // #2
    // COND:
    // ( §FS_LED_RGB_DX<>§SI ) AND ( §FS_LED_PER_DX<>§SI )
    // THEN:
    // *SET *CF=#0
    // #3
    // COND:
    // ( §FS_LED_RGB_DX=§SI ) OR ( §FS_LED_PER_DX=§SI )
    // THEN:
    // *SET *CF=#1
    // #4
    // COND:
    ...
    //#2
    if (vars.get('§FS_LED_RGB_DX') != vars.get('§SI') && vars.get('§FS_LED_PER_DX') != vars.get('§SI')) {
        vars.setCF(0);
    }
