...
export const REG0004699: Rule = (iv) => {
    ...
    // #1
    // COND:
    // ( §L_SPO_MON_ANT1 > #0 ) OR (§FS_MON_ANT1<>§SI ) OR (§L_SBA_ANT1 >#0)
    // THEN:
    ...
    // RESPONSE:
    //#1
    if ((vars.get('§L_SPO_MON_ANT1') > 0) || (vars.get('§FS_MON_ANT1') != vars.get('§SI')) || (vars.get('§L_SBA_ANT1') > 0)) {
