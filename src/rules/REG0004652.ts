...
export const REG0004652: Rule = (iv) => {
    ...
    // #2
    // COND:
    // ( §FS_SCA_MON_ANT2<>§SI ) AND ( §FS_ACC_CEN<>§SI ) AND ( §FS_ACC_PRI<>§SI ) AND ( §FS_ACC_PRI_02<>§SI )                         AND ( §FS_ACC_FIN<>§SI )
    // THEN:
    // *SET *CF=#1
    // #3
    // COND:
    // ( §FS_SCA_MON_ANT2<>§SI ) AND (§FS_ACC_FIN=§SI )
    // THEN:
    // *SET *CF=#1
    // """
    // RESPONSE:
