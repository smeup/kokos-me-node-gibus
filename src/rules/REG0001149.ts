...
export const REG0001149: Rule = (iv) => {
    ...
    // #2
    // COND:
    // (§FS_SCRIGNO_300 = §SI) OR (§FS_SCRIGNO_250 = §SI)
    // THEN:
    ...
    // #5
    // COND:
    // §VERN_1_COM = '0065' OR §VERN_1_COM = '0053'
    // THEN:
    // *SET *CON-A = 'P055'
    // #6
    // COND:
    ...
    //#2
    if (vars.get('§FS_SCRIGNO_300') == vars.get('§SI') || vars.get('§FS_SCRIGNO_250') == vars.get('§SI')) {
    ...
    //#5
    if (vars.get('§VERN_1_COM') == '0065' || vars.get('§VERN_1_COM') == '0053') {
        vars.setCON_A('P055');
    }
