...
export const REG0004008: Rule = (iv) => {
    ...
    // #1
    // COND:
    // (( §H_POS_M2=#0 ) OR (( §FS_ACC_PRI = §SI ) AND ( §LA_ADD_LAT='SX'))) AND ( §FS_FAM2_ADDL= §SI )
    // THEN:
    // *SET *CF =#0
    // #2
    // COND:
    ...
    //#1
    if (((vars.get('§H_POS_M2') == 0) || ((vars.get('§FS_ACC_PRI') == vars.get('§SI')) && (vars.get('§LA_ADD_LAT') == 'SX'))) && (vars.get('§FS_FAM2_ADDL') == vars.get('§SI'))) {
        vars.setCF(0);
    }
    //#2
