...
export const REG0001064: Rule = (iv) => {
    ...
    // THEN:
    // *SET *CON-B = [§L_GUIDA_ZIP + §AUM_GUI_DX]
    // *SET *CF = #1
    // #3
    // COND:
    ...
    // §FS_CLI_ZIP_TWI = §SI
    // THEN:
    // *SET *CON-B = [§ALTE_TC - #24,8 + §AUM_GUI_DX]
    // #5
    // COND:
    // §FS_CLI_TWI_BG = §SI
    // THEN:
    ...
    if (vars.get('§AUM_GUI_DX') != vars.get('§AUM_GUI_SX')) {
        vars.setCON_B(vars.get('§L_GUIDA_ZIP') + vars.get('§AUM_GUI_DX'));
        vars.setCF(1);
    }
    ...
    if (vars.get('§FS_CLI_ZIP_TWI') == vars.get('§SI')) {
        vars.setCON_B(vars.get('§ALTE_TC') - 24.8 + vars.get('§AUM_GUI_DX'));
    }
