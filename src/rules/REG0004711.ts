...
export const REG0004711: Rule = (iv) => {
    ...
    // #8
    // COND:
    // ( §FS_FAM2_ADDL=§SI ) AND (§TI_ACC_TWI='02') AND ((§FS_ACC_CEN=§SI ) OR (§FS_ACC_FIN=§SI)) AND (§LA_ADD_LAT='DX')
    // THEN:
    // *SET *CF=#0
    // #9
    // COND:
    ...
    //#8
    if ((vars.get('§FS_FAM2_ADDL') == vars.get('§SI')) && (vars.get('§TI_ACC_TWI') == '02') && ((vars.get('§FS_ACC_CEN') == vars.get('§SI')) || (vars.get('§FS_ACC_FIN') == vars.get('§SI'))) && (vars.get('§LA_ADD_LAT') == 'DX')) {
        vars.setCF(0);
    }
