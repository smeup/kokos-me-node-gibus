...
export const REG0005208: Rule = (iv) => {
    ...
    // #1
    // COND:
    // ( (§L_SPO_MON_ANT1 = #0 ) AND (§FS_MON_ANT1=§SI ) AND (§L_SBA_ANT1 =#0) ) AND ( §FS_FAM2_ISOLA=§SI )
    // THEN:
    // *SET *CON-A=*COL
    // *SET *CON-B=*LUNG
    // *SET *S2=*LUNG
    ...
    // RESPONSE:
    //#1
    if ((vars.get('§L_SPO_MON_ANT1') == 0) && (vars.get('§FS_MON_ANT1') == vars.get('§SI') && (vars.get('§L_SBA_ANT1') == 0)) && (vars.get('§FS_FAM2_ISOLA') == vars.get('§SI'))) {
