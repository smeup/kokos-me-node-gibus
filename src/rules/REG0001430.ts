...
export const REG0001430: Rule = (iv) => {
    ...
    // #12
    // COND:
    // §L_PORTA_ZIP > #0 AND §AUM_GUI_SX > #0
    // THEN:
    // *SET §DUMMYN1  = [§L_PORTA_ZIP + §AUM_GUI_SX]
    // *SET *CF = #1
    // #13
    // THEN:
    // *SET *CON-B = §DUMMYN1
    ...
    //#11
    if (vars.get('§L_PORTA_ZIP') > 0) {
        vars.setDUMMYN1(vars.get('§L_PORTA_ZIP'));
        vars.setCF(1);
    }
    //#12
    if (vars.get('§L_PORTA_ZIP') > 0 && vars.get('§AUM_GUI_SX') > 0) {
        vars.setDUMMYN1(vars.get('§L_PORTA_ZIP') + vars.get('§AUM_GUI_SX'));
        vars.setCF(1);
    }
    //#13
