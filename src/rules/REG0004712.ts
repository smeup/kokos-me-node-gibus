...
export const REG0004712: Rule = (iv) => {
    ...
    // #14
    // COND:
    // ( §FS_FAM2_ADDL=§SI ) AND (§LA_ADD_LAT='DX') AND ((§FS_ACC_PRI=§SI ) AND (§FS_ACC_PRI_02=§SI ))
    // THEN:
    // *SET *CF=#0
    // #15
    // COND:
    // ( §FS_FAM2_ADDL=§SI ) AND (( §LA_ADD_LAT='SX' ) OR ( §LA_ADD_LAT='DX' )) AND ( §FS_ACC_PRI_02=§SI )
    // THEN:
    // *SET *CF=#0
    // #16
    // COND:
    ...
    //#15
    if ((vars.get('§FS_FAM2_ADDL') == vars.get('§SI')) && ((vars.get('§LA_ADD_LAT') == 'SX') || (vars.get('§LA_ADD_LAT') == 'DX')) && (vars.get('§FS_ACC_PRI_02') == vars.get('§SI'))) {
        vars.setCF(0);
    }
