...
export const REG0004091: Rule = (iv) => {
    ...
    // #4
    // COND:
    // (§FS_FAM2_ADDL = §SI ) AND ( §FS_ACC_PRI <> §SI ) AND ( §LA_ADD_LAT = 'SX' ) AND ( §FS_SCA_MON_POS2 = §SI )
    // THEN:
    // *SET *CF= #1
    // #5
    // COND:
    ...
    //#4
    if ((vars.get('§FS_FAM2_ADDL') == vars.get('§SI')) && (vars.get('§FS_ACC_PRI') != vars.get('§SI')) && (vars.get('§LA_ADD_LAT') == 'SX') && (vars.get('§FS_SCA_MON_POS2') == vars.get('§SI'))) {
        vars.setCF(1);
    }
