...
export const REG0000244: Rule = (iv) => {
    ...
    // #2
    // COND:
    // (§FS_DIM_250 = §SI) OR (§FS_COMBI_250 = §SI) OR (§FS_DIM_300 = §SI)
    // THEN:
    // *SET *CF = #1
    // #3
    // COND:
    ...
    //#2
    if (vars.get('§FS_DIM_250') == vars.get('§SI') || vars.get('§FS_COMBI_250') == vars.get('§SI') || vars.get('§FS_DIM_300') == vars.get('§SI')) {
        vars.setCF(1);
    }
