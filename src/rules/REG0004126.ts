...
export const REG0004126: Rule = (iv) => {
    ...
    // #2
    // COND:
    // (§FS_FAM2_ADDL=§SI) AND (( §FS_ACC_PRI=§SI ) OR (§FS_ACC_PRI_02=§SI ))
    // THEN:
    // *SET *CF =#1
    // #3
    // COND:
    ...
    vars.setCF(0);
    //#2
    if ((vars.get('§FS_FAM2_ADDL') == vars.get('§SI')) && ((vars.get('§FS_ACC_PRI') == vars.get('§SI')) || (vars.get('§FS_ACC_PRI_02') == vars.get('§SI')))) {
        vars.setCF(1);
    }
    //#3
