...
export const REG0004197: Rule = (iv) => {
    ...
    // #3
    // COND:
    // ( §FS_STA_ACC_POS=§SI ) AND ( §FS_ACC_FIN = §SI )
    // THEN:
    // *SET *CF=#0
    // #4
    // COND:
    ...
    //#2
    if (vars.get('§FS_STA_ACC_POS') != vars.get('§SI')) {
        vars.setCF(0);
    } else {
        vars.setCF(1);
    }
    ...
    //#3
    if ((vars.get('§FS_STA_ACC_POS') == vars.get('§SI')) && (vars.get('§FS_ACC_FIN') == vars.get('§SI'))) {
        vars.setCF(0);
    }
