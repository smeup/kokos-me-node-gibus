/**
 * This rule represents a template implementation of a rule.
 * It takes an input value and performs some operations on it using the Variables class.
 * The result is returned as the output value.
 * 
 * @param iv The input value for the rule.
 * @returns The output value after applying the rule.
 */
import { Rule } from "../types/general";
import { Variables } from "../converter/variables";

export const REG0008248: Rule = (iv) => {

    const vars = new Variables(iv);

    // GENERATED
    // RULE: REG0008248
    // REQUEST:
    // """
    // #1
    // THEN:
    // *SET *CF=#0
    // #2
    // COND:
    // ( §SPOR >=#700 ) AND ( §FS_FAM2_ISOLA=§SI )
    // THEN:
    // *SET *CF=#2
    // #3
    // COND:
    // ( §SPOR >=#700 ) AND ( §FS_FAM2_ADDF=§SI )
    // THEN:
    // *SET *CF=#2
    // #4
    // COND:
    // ( §SPOR >=#700 ) AND ( §FS_FAM2_ADDL=§SI )
    // THEN:
    // *SET *CF=#1
    // #5
    // COND:
    // ( §SPOR >= #660 ) AND ( §FS_FAM2_ISOLA = §SI ) AND ( §N_MODULI = #2 )
    // THEN:
    // *SET *CF = #2
    // """
    // RESPONSE:
    //#1
    vars.setCF(0);
    
    //#2
    if ((vars.get('§SPOR') >= 700) && (vars.get('§FS_FAM2_ISOLA') == vars.get('§SI'))) {
        vars.setCF(2);
    }
    
    //#3
    if ((vars.get('§SPOR') >= 700) && (vars.get('§FS_FAM2_ADDF') == vars.get('§SI'))) {
        vars.setCF(2);
    }
    
    //#4
    if ((vars.get('§SPOR') >= 700) && (vars.get('§FS_FAM2_ADDL') == vars.get('§SI'))) {
        vars.setCF(1);
    }
    
    //#5
    if ((vars.get('§SPOR') >= 660) && (vars.get('§FS_FAM2_ISOLA') == vars.get('§SI') && (vars.get('§N_MODULI') == 2))) {
        vars.setCF(2);
    }
    // GENERATED

    return vars.output;
};
