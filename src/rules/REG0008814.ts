/**
 * This rule represents a template implementation of a rule.
 * It takes an input value and performs some operations on it using the Variables class.
 * The result is returned as the output value.
 * 
 * @param iv The input value for the rule.
 * @returns The output value after applying the rule.
 */
import { Rule } from "../types/general.js";
import { Variables } from "../converter/variables.js";

export const REG0008814: Rule = (iv) => {

    const vars = new Variables(iv);

    // GENERATED
    // RULE: REG0008814
    // REQUEST:
    // """
    // #1
    // THEN:
    // *SET *CF = #0
    // *SET *CON-A = §VERN_1_COM
    // *SET §DUMMYB1 = ''
    // #2
    // COND:
    // §FS_FAM2_ZENIT = §SI
    // THEN:
    // *SET §DUMMYB1 = §SI
    // *SET §DUMMYN1 = #24
    // *SET §DUMMYN2 = #24
    // *SET §DUMMYN3 = #24
    // #3
    // COND:
    // §FS_FAM2_ZENIT = §SI AND §VER_STA_T01 = '2021'
    // THEN:
    // *SET §DUMMYB1 = §SI
    // *SET §DUMMYN1 = #17
    // *SET §DUMMYN2 = #17
    // *SET §DUMMYN3 = #17
    // #4
    // COND:
    // §FS_FAM2_AZIMUT = §SI
    // THEN:
    // *SET §DUMMYB1 = §SI
    // *SET §DUMMYN1 = #25
    // *SET §DUMMYN2 = #23,1
    // *SET §DUMMYN3 = #21,3
    // #5
    // COND:
    // §FS_FAM2_AZIMUT = §SI AND §VER_STA_T01 = '2021'
    // THEN:
    // *SET §DUMMYB1 = §SI
    // *SET §DUMMYN1 = #18
    // *SET §DUMMYN2 = #16,1
    // *SET §DUMMYN3 = #14,2
    // #6
    // COND:
    // §DUMMYB1 = §SI
    // THEN:
    // *SET *CF = #1
    // *SET *CON-B = [§SPOR - §DUMMYN1]
    // #7
    // COND:
    // §DUMMYB1 = §SI AND (§FS_ACC_LAT_ANT = §SI OR §FS_ACC_LAT_POS = §SI)
    // THEN:
    // *SET *CF = #1
    // *SET *CON-B = [§SPOR - §DUMMYN2]
    // #8
    // COND:
    // §DUMMYB1 = §SI AND (§FS_ACC_LAT_ANT = §SI AND §FS_ACC_LAT_POS = §SI)
    // THEN:
    // *SET *CF = #1
    // *SET *CON-B = [§SPOR - §DUMMYN3]
    // #9
    // COND:
    // §VER_SCO_T01 = '2020' AND §FS_ACC_LAT_DX = §SI
    // THEN:
    // *SET *CF = #0
    // """
    // RESPONSE:
    //#1
    vars.setCF(0);
    vars.setCON_A(vars.get('§VERN_1_COM'));
    vars.setDUMMYB1('');
    //#2
    if (vars.get('§FS_FAM2_ZENIT') == vars.get('§SI')) {
        vars.setDUMMYB1(vars.get('§SI'));
        vars.setDUMMYN1(24);
        vars.setDUMMYN2(24);
        vars.setDUMMYN3(24);
    }
    //#3
    if (vars.get('§FS_FAM2_ZENIT') == vars.get('§SI') && vars.get('§VER_STA_T01') == '2021') {
        vars.setDUMMYB1(vars.get('§SI'));
        vars.setDUMMYN1(17);
        vars.setDUMMYN2(17);
        vars.setDUMMYN3(17);
    }
    //#4
    if (vars.get('§FS_FAM2_AZIMUT') == vars.get('§SI')) {
        vars.setDUMMYB1(vars.get('§SI'));
        vars.setDUMMYN1(25);
        vars.setDUMMYN2(23.1);
        vars.setDUMMYN3(21.3);
    }
    //#5
    if (vars.get('§FS_FAM2_AZIMUT') == vars.get('§SI') && vars.get('§VER_STA_T01') == '2021') {
        vars.setDUMMYB1(vars.get('§SI'));
        vars.setDUMMYN1(18);
        vars.setDUMMYN2(16.1);
        vars.setDUMMYN3(14.2);
    }
    //#6
    if (vars.get('§DUMMYB1') == vars.get('§SI')) {
        vars.setCF(1);
        vars.setCON_B(vars.get('§SPOR') - vars.get('§DUMMYN1'));
    }
    //#7
    if (vars.get('§DUMMYB1') == vars.get('§SI') && (vars.get('§FS_ACC_LAT_ANT') == vars.get('§SI') || vars.get('§FS_ACC_LAT_POS') == vars.get('§SI'))) {
        vars.setCF(1);
        vars.setCON_B(vars.get('§SPOR') - vars.get('§DUMMYN2'));
    }
    //#8
    if (vars.get('§DUMMYB1') == vars.get('§SI') && (vars.get('§FS_ACC_LAT_ANT') == vars.get('§SI') && vars.get('§FS_ACC_LAT_POS') == vars.get('§SI'))) {
        vars.setCF(1);
        vars.setCON_B(vars.get('§SPOR') - vars.get('§DUMMYN3'));
    }
    //#9
    if (vars.get('§VER_SCO_T01') == '2020' && vars.get('§FS_ACC_LAT_DX') == vars.get('§SI')) {
        vars.setCF(0);
    }
    // GENERATED

    return vars.output;
};
