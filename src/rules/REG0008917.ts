...
export const REG0008917: Rule = (iv) => {
    ...
    // #6
    // COND:
    // §FS_STA_MUR_POS1 = §SI OR §FS_ST_MU_P1_ACC = §SI
    // THEN:
    // *SET §DUMMYB2 = ''
    // #7
    // THEN:
    ...
    //#6
    if (vars.get('§FS_STA_MUR_POS1') == vars.get('§SI') || vars.get('§FS_ST_MU_P1_ACC') == vars.get('§SI')) {
        vars.setDUMMYB2("");
    }
    //#7
    vars.setCF(1);
    vars.setDUMMYN3(vars.get('§SPOR') - 30);
    vars.setDUMMYN1(7.5);
    //#8
