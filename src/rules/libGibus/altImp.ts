...
export const altImpT01 = async (data: any) => {
  ...
  configList = createConfigObjList(queryResult, nFrazionamenti);
...
function calcAltImp(data: any, lungObjList: tLungObjList, lung: number, lIntestazione: number) {
    ...
    let bestObjElem = lungObjList.find((lungObjElem) => ((lungObjElem.lungDiv - lIntestazione) >= lung));
...
function calcDefLungObjList(filtConfigList: tConfigList) {
  ...
  filtConfigList.forEach((filtElem) => {
    filtElem.lungObjList.forEach((lungObjElem) => {
      let found = defLungObjList.find((defLungObjElem) => defLungObjElem === lungObjElem);
...
function createConfigObjList(queryResult: any, nFrazionamenti: number) {
  let configList: tConfigList = [];
  ...
  queryResult.forEach((queryElem: any) => {
    ...
    configList.forEach((configElem) => {
      if (configElem.col === colQueryElem) {
        configElem.lungObjList = populateLungObjList(config.lungObjList, lungQueryElem, nFrazionamenti);
        found = true;
      }
    })

    if (!found) {
      configList.push({ col: colQueryElem, lungObjList: populateLungObjList([], lungQueryElem, nFrazionamenti) });
    }
  });
...
function populateLungObjList(lungObjList: tLungObjList, lung: number, nFrazionamenti: number) {
  let additional = [];
  for (let index = nFrazionamenti; index >= 1; index--) {
    additional.push({ lungPR: lung, lungDiv: lung / index, div: index });
