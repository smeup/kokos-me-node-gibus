...
type gtechStatusType = { validVersion: boolean, validData: boolean, errorListSent: boolean, record: any };
type childObjType = { level: Number, codArt: string, pos: string, parentPos: string };
...
function initGtechStatus() {
    let gtechStatus: gtechStatusType = { validVersion: false, validData: true, errorListSent: false, record: {} };
    return gtechStatus;
}
...
async function updateGtechRecord(connection: Connection, gtechBomObj: any, metaDataObj: any, codCfg: string, timestamp: string, bomVersion: string) {
    ...
    if (metaDataObj) updateString += `JSONMETADATA = '${JSON.stringify(metaDataObj)}'`;
    updateString += "WHERE CODCFG = ? AND TIMESTAMP = ? AND BOMVERSION = ?";

    updateResult = await connection.update(updateString, [codCfg, timestamp, bomVersion]);

    return updateResult;
}
async function insertGtechRecord(connection: Connection, gtechBomObj: gtechBomObjType, metaDataObj: any, codCfg: string, timestamp: string, bomVersion: string) {
    ...
    insertResult = await connection.update(insertString, [{ type: 'CLOB', value: jsonDoc }, { type: 'CLOB', value: jsonMetadata }, codCfg, timestamp, bomVersion, 1]);


    return insertResult;
}

async function getGtechRecord(connection: Connection, codCfg: string, timestamp: string, bomVersion: string) {
    ...
    selectResult = await connection.query(selectString, [codCfg, bomVersion, timestamp]);
...
function chkBomProps(bomItemValue: string, linkData: any, propsList: string[]) {
    let result = false;
    for (const prop of propsList) {//almeno una deve matchare
...
function validateLink(linkData: any, bom: bomItemType[], notValidGtechBom: boolean) {

    if (notValidGtechBom) return linkData['XFVALI'] = '';

    //*POS, come potenzialmente *COL e *LUNG arriva popolata da INITDATAOBJ

    let keyFieldMapList = [{ "type": "onlyIfPresent", "smeup": ["D§USR1"], "gtech": "seq1" },
    ...
    let bomItem = bom.find((bomItem: any) => {
        ...
        for (const keyFieldMap of keyFieldMapList) {
            ...
            if (gotToCheck) if (!chkBomProps(bomItem[keyFieldMap.gtech], linkData, keyFieldMap.smeup)) found = false;
        }
        return found;
    }) as bomItemType;
...
async function loadTranscodeList() {
    ...
    for (const fileName of fileNamesList) {
        ...
        let theJson = (await import("./" + fileName, {
            with: { type: 'json' }
        })).default;
...
function calc003config(colPolicy: string, linkData: any) {
    ...
    let valueToSet: any = '';
    let vrObj: {
        VRSTR: string, VRLAMFRA: string, VRPLASTR: string, VRPLALAMFRA: string,
        VRLAMIERASTR: string, VRSPECIAL01: string, VRABBSTR: string, VRABBLAMFRA: string
    } = {
        VRSTR: '', VRPLASTR: '', VRLAMFRA: '', VRPLALAMFRA: '', VRLAMIERASTR: '', VRSPECIAL01: '',
        VRABBSTR: '', VRABBLAMFRA: ''
    };
    ...
    switch (colPolicy.toUpperCase()) {
        ...
        case 'VRPLALAMFRA':
        case 'VRLAMIERASTR':
            let index: keyof typeof vrObj = colPolicy.toUpperCase() as keyof typeof vrObj;
        ...
        case 'VRSPECIAL01'://abbinata al criterio 003A per azzerari i pezzi
            ...
            if (['0053', '0055', '0059', '0065', '0069', '0070', '0073', '0076', '0103',
                '0107', '0121', '0127', '0130', '0131', '0147', '0616', '0172', '0208',
                '0224', '0258', '0266', '0617', '0280', '0285', '0293', '0316', '0618',
                '0323', '0324', '0619', '0620', '0621'].includes(vrObj.VRSTR)) {
...
type rawItemType = { index: number, name: string, smeup: string, counter: number, ref: string, codifica: string, lunghezza: number };
...
type transcodeItemType = { cod: string, blackListed: boolean, transCod: string, forceSumQty: boolean };

function createBomObj(rawBomList: rawItemType[], transcodeList: transcodeItemType[]) {
    let retBomObj: gtechBomObjType = { bomList: [], errorList: [] };
    let bomItemList: bomItemType[] = [];

    for (const rawItem of rawBomList) {
        if (rawItem.counter) {
            let transcodeItem = transcodeList.find((item: transcodeItemType) => rawItem.smeup === item.cod) as transcodeItemType;
...
type bomItemType = { cod: string, ref: string, codifica: string, pezzi: number, lunghezza: number, forceSumQty: boolean };
...
function calcBomItem(rawItem: rawItemType, transcodeItem: transcodeItemType) {
    ...
    if (transcodeItem) {
        ...
        if (transcodeItem.forceSumQty) bomItem.forceSumQty = transcodeItem.forceSumQty;
    }
...
function createChildObj(level: number, codArt: string, pos: string, parentPos: string) {
    let childObj: childObjType = { level: level, codArt: codArt, pos: pos, parentPos: parentPos };
    return childObj;
}

async function getBomChildrenList(connection: Connection, codBom: string, maxLevel: number) {
    ...
    for (let index = 1; index < maxLevel; index++) {
        let childrenListLevel = bomChildrenList.filter(elem => elem.level === index);
        for (const element of childrenListLevel) {
            let codArt = element.codArt;
            let parentPos = element.pos;//ma cosa sto scrivendo?
            bomChildrenList = bomChildrenList.concat(await getChildrenList(connection, codArt, index + 1, parentPos));
        }
    }
    return bomChildrenList;
}

async function getChildrenList(connection: Connection, codArt: string, level: number, parentPos: string) {
    ...
    for (let element of selectResult) {
        let childrenObj: childObjType = { level: level, codArt: element['D§COMP'], pos: element['D§PAS2'], parentPos: parentPos };
...
async function chkSmeupBom(connection: Connection, gtechBomObj: gtechBomObjType, codArt: string) {
    ...
    for (const element of gtechBomList) {
        let found = true;
        let firstChildrenList = bomChildrenList.filter(elem => elem.codArt === element.cod);
        if (!firstChildrenList) {
            found = false;
        }
        else
            if (element.ref) { //do per scontato che... adesso usiamo il ref
                let refFound = false;
                if (firstChildrenList.find(elem => elem.parentPos === element.ref)) refFound = true;
                if (firstChildrenList.find(elem => elem.pos === element.ref)) refFound = true;
...
async function manageGtechBom(linkData: RuleVariableMap, bomVersion: string, calcParObj: any, bomClass: any) {
    cleanData(linkData);
    ...
    let gtechStatus = await manageGtechStatus(connection, codCfg, timestamp, bomVersion);

    if (!gtechStatus.validVersion) {
        let gtechBomObj = await createGtechBom(linkData, codCfg, timestamp, calcParObj, bomClass);
        let jsonMetadata: any = { errorList: [] };
        if (gtechBomObj.errorList.length > 0) {
            jsonMetadata.errorList = gtechBomObj.errorList;
        } else {//non ci sono errori alla base della distinta gibustech
            let chkBomObj: smeupChkObjType = await chkSmeupBom(connection, gtechBomObj, linkData['_AR']);
            if (chkBomObj.notFoundInSmeupList) {
                if (chkBomObj.notFoundInSmeupList.length > 0) {
                    jsonMetadata.errorList.push({ notFoundInSmeup: chkBomObj.notFoundInSmeupList });
                }
            }
        }
        let result = await insertGtechRecord(connection, gtechBomObj, jsonMetadata, codCfg, timestamp, bomVersion);
        gtechStatus = await manageGtechStatus(connection, codCfg, timestamp, bomVersion);
    }
...
async function manageGtechStatus(connection: Connection, codCfg: string, timestamp: string, bomVersion: string) {
    let gtechStatus = initGtechStatus();
    let gtechRecord = await getGtechRecord(connection, codCfg, timestamp, bomVersion);
    ...
    if (gtechRecord) {
        ...
        jsonMetadata = gtechRecord['JSONMETADATA'] ? JSON.parse(gtechRecord['JSONMETADATA']) : '';
        if (jsonMetadata) {
            ...
            if (gtechRecord['BOMVERSION'] === bomVersion) gtechStatus.validVersion = true;
        }
    }
    ...
    if (gtechStatus.validVersion) {
        if (!gtechStatus.validData) {
            if (!gtechStatus.errorListSent) {
                ...
                updateGtechRecord(connection, "", jsonMetadata, codCfg, timestamp, bomVersion);
            };
        }
    }
...
async function createGtechBom(linkData: RuleVariableMap, codCfg: string, timestamp: string, calcParObj: any, bomClass: any) {
